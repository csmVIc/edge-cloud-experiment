...
"""
    ...
    "-pt", "--partition"  手动设置分层点
"""
if __name__ == '__main__':
    try:
        opts, args = getopt.getopt(sys.argv[1:], "t:i:p:d:pt:", ["type=","ip=","port=","device_on=","partition="])
    ...
    for opt, arg in opts:
        if opt in ("-t", "--type"):
            ...
            device = arg
        elif opt in ("-pt", "--partition"):
            partition_manual = arg
    ...
    if(partition_manual is not None):
        partition_point = int(partition_manual)
        print(f"手动设置切分点: partition_point = {partition_point} ")
    print(f"最终切分点: partition_point = {partition_point} ")
    # 使用云边协同的方式进行模拟
