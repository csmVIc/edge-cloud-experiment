...
def start_client(ip,port,input_x,model_type,partition_point,device):
    ...
    # 打印出中间特征值大小
    print(f"中间特征值占用字节 edge_output.nbytes = {edge_output.element_size() * edge_output.numel()} bytes")
    print(f"中间特征值序列化后大小 = {len(pickle.dumps(edge_output))} bytes")
    # 发送中间数据
    ...
    print(f"端到端推理时间 - {edge_latency + transfer_latency + cloud_latency:.3f} ms")
    print("================= DNN Collaborative Inference Finished. ===================")
    conn.close()
